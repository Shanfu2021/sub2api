...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	agentEnterpriseDeletionCleanupRepository := service.ProvideAgentEnterpriseDeletionCleanupRepository(enterpriseManagementRepository)
	usageLogRepository := repository.NewUsageLogRepository(client, db)
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator)
	agentManagementService := service.ProvideAgentManagementService(agentManagementRepository, userRepository, groupRepository, apiKeyAuthCacheInvalidator, agentEnterpriseDeletionCleanupRepository, userGroupRateRepository, settingRepository, usageService)
	...
	agentUserDeletionCleanupRepository := service.ProvideAgentUserDeletionCleanupRepository(agentManagementRepository)
	enterpriseAdminCleanupRepository := service.ProvideEnterpriseAdminCleanupRepository(enterpriseManagementRepository)
	proxyExitInfoProber := repository.NewProxyExitInfoProber(configConfig)
	proxyLatencyCache := repository.NewProxyLatencyCache(redisClient)
	adminService := service.NewAdminService(userRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, agentUserDeletionCleanupRepository, enterpriseAdminCleanupRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService)
	...
	enterpriseManagementHandler := handler.NewEnterpriseManagementHandler(enterpriseManagementService)
	...
	purchaseInfoHandler := handler.NewPurchaseInfoHandler(purchaseInfoService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, agentManagementHandler, enterpriseManagementHandler, purchaseInfoHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
