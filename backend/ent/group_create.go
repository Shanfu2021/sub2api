...
func (_c *GroupCreate) AddAgentGroupDelegationIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddAgentGroupDelegationIDs(ids...)
	return _c
}

// AddAgentGroupDelegations adds the "agent_group_delegations" edges to the AgentGroupDelegation entity.
func (_c *GroupCreate) AddAgentGroupDelegations(v ...*AgentGroupDelegation) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddAgentGroupDelegationIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.AgentGroupDelegationsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
