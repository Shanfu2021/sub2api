...
type GroupQuery struct {
	...
	withAgentGroupDelegations *AgentGroupDelegationQuery
...
// QueryAgentGroupDelegations chains the current query on the "agent_group_delegations" edge.
func (_q *GroupQuery) QueryAgentGroupDelegations() *AgentGroupDelegationQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAgentGroupDelegations: _q.withAgentGroupDelegations.Clone(),
...
// WithAgentGroupDelegations tells the query-builder to eager-load the nodes that are connected to
// the "agent_group_delegations" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAgentGroupDelegations(opts ...func(*AgentGroupDelegationQuery)) *GroupQuery {
	query := (&AgentGroupDelegationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAgentGroupDelegations = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	var (
		...
		loadedTypes = [9]bool{
			...
			_q.withAgentGroupDelegations != nil,
	...
	if query := _q.withAgentGroupDelegations; query != nil {
		if err := _q.loadAgentGroupDelegations(ctx, query, nodes,
			func(n *Group) { n.Edges.AgentGroupDelegations = []*AgentGroupDelegation{} },
			func(n *Group, e *AgentGroupDelegation) {
				n.Edges.AgentGroupDelegations = append(n.Edges.AgentGroupDelegations, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAgentGroupDelegations(ctx context.Context, query *AgentGroupDelegationQuery, nodes []*Group, init func(*Group), assign func(*Group, *AgentGroupDelegation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agentgroupdelegation.FieldGroupID)
	}
	query.Where(predicate.AgentGroupDelegation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AgentGroupDelegationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
