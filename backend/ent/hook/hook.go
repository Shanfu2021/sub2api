...
// The AgentGroupDelegationFunc type is an adapter to allow the use of ordinary
// function as AgentGroupDelegation mutator.
type AgentGroupDelegationFunc func(context.Context, *ent.AgentGroupDelegationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AgentGroupDelegationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AgentGroupDelegationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AgentGroupDelegationMutation", m)
}
