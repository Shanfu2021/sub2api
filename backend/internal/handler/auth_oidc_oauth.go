...
func (h *AuthHandler) CompleteOIDCOAuthRegistration(c *gin.Context) {
	...
	tokenPair, user, err := h.authService.LoginOrRegisterOAuthWithTokenPair(c.Request.Context(), email, username, req.InvitationCode, req.AffCode, ip.GetTrustedClientIP(c), "oidc")
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) tryOIDCVerifiedEmailFastPath(
	...
	tokenPair, _, err := h.authService.LoginOrRegisterVerifiedEmailOAuthWithInvitation(ctx, input, "", "", ip.GetTrustedClientIP(c))
	if err != nil {
