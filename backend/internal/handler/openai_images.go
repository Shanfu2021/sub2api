...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	userReleaseFunc, acquired := h.acquireResponsesUserSlot(c, subject.UserID, subject.Concurrency, subject, parsed.Stream, &streamStarted, reqLog)
	if !acquired {
		return
	}
	...
	for {
		...
		if result != nil {
			...
			h.gatewayService.MaybeTempUnscheduleSlowFirstToken(c.Request.Context(), account, result.FirstTokenMs, parsed.Model)
