...
func appendUsageLogUserScopeWhereCondition(conditions []string, args []any, column string, userID int64, userIDs []int64) ([]string, []any) {
	if userID > 0 {
		...
		args = append(args, pq.Array(normalizePositiveInt64IDs(userIDs)))
	}
	return conditions, args
}

func appendUsageLogUserScopeQueryFilter(query string, args []any, column string, userID int64, userIDs []int64) (string, []any) {
	conditions, args := appendUsageLogUserScopeWhereCondition(nil, args, column, userID, userIDs)
	if len(conditions) == 0 {
		return query, args
	}
	return query + " AND " + conditions[0], args
