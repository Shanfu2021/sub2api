...
func (s *UsageLogRepoSuite) TestGetByID_RoundTripsAgentSnapshotAndNegativeIncome() {
	user := mustCreateUser(s.T(), s.client, &service.User{Email: "getbyid-agent-snapshot@test.com"})
	owner := mustCreateUser(s.T(), s.client, &service.User{Email: "getbyid-agent-owner@test.com"})
	apiKey := mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: user.ID, Key: "sk-getbyid-agent-snapshot", Name: "k"})
	account := mustCreateAccount(s.T(), s.client, &service.Account{Name: "acc-getbyid-agent-snapshot"})
	...
	got, err := s.repo.GetByID(s.ctx, log.ID)
	s.Require().NoError(err)
	s.Require().NotNil(got.AgentOwnerUserID)
	s.Require().Equal(owner.ID, *got.AgentOwnerUserID)
	s.Require().InEpsilon(0.8, got.AgentUserRateMultiplier, 0.0001)
	s.Require().InEpsilon(1.2, got.AgentCostRateMultiplier, 0.0001)
	s.Require().InEpsilon(-0.25, got.AgentIncome, 0.0001)
}
