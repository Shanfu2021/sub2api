...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	const imageSizeArgIndex = 38
	require.Equal(t, sql.NullString{String: imageSize, Valid: true}, prepared.args[imageSizeArgIndex])
	require.Equal(t, sql.NullString{String: inputSize, Valid: true}, prepared.args[imageSizeArgIndex+1])
	require.Equal(t, sql.NullString{String: outputSize, Valid: true}, prepared.args[imageSizeArgIndex+2])
	require.Equal(t, sql.NullString{String: source, Valid: true}, prepared.args[imageSizeArgIndex+3])
	breakdownJSON, ok := prepared.args[imageSizeArgIndex+4].(string)
	require.True(t, ok)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			0.9,               // actual_cost
			1.0,               // rate_multiplier
			sql.NullInt64{},   // agent_owner_user_id
			0.0,               // agent_user_rate_multiplier
			0.0,               // agent_cost_rate_multiplier
			0.0,               // agent_income
