...
func (s *adminServiceImpl) ClearGroupRateMultipliers(ctx context.Context, groupID int64) error {
	...
	// Delegated/user-specific group rates are embedded in auth cache snapshots.
	// Clear the group cache so affected API keys reload the effective multiplier.
	if s.authCacheInvalidator != nil {
...
func (s *adminServiceImpl) BatchSetGroupRateMultipliers(ctx context.Context, groupID int64, entries []GroupRateMultiplierInput) error {
	...
	// Delegated/user-specific group rates are embedded in auth cache snapshots.
	// Invalidate after sync so the next request observes the new multiplier.
	if s.authCacheInvalidator != nil {
		s.authCacheInvalidator.InvalidateAuthCacheByGroupID(ctx, groupID)
	}
	return nil
