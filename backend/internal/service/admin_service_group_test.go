...
func TestAdminService_CreateGroup_PersistsStrictPriorityScheduling(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.Equal(t, GroupSchedulingStrategyStrictPriority, repo.created.SchedulingStrategy)
}

func TestAdminService_UpdateGroup_PersistsSchedulingStrategy(t *testing.T) {
	...
	svc := &adminServiceImpl{groupRepo: repo}
	strategy := GroupSchedulingStrategyStrictPriority

	group, err := svc.UpdateGroup(context.Background(), 42, &UpdateGroupInput{SchedulingStrategy: &strategy})

	require.NoError(t, err)
	require.NotNil(t, group)
	require.Equal(t, GroupSchedulingStrategyStrictPriority, repo.updated.SchedulingStrategy)
}

func TestAdminService_CreateGroup_RejectsInvalidSchedulingStrategy(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.EqualError(t, err, "invalid scheduling_strategy")
