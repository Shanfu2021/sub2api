...
type agentManagementSettingRepoStub struct {
	values map[string]string
	sets   map[string]string
}

func (r *agentManagementSettingRepoStub) Get(context.Context, string) (*Setting, error) {
	panic("unexpected Get call")
}

func (r *agentManagementSettingRepoStub) GetValue(_ context.Context, key string) (string, error) {
	if r.values == nil {
...
func (r *agentManagementSettingRepoStub) Set(_ context.Context, key, value string) error {
	if r.sets == nil {
...
func (r *agentManagementSettingRepoStub) GetMultiple(context.Context, []string) (map[string]string, error) {
	panic("unexpected GetMultiple call")
}

func (r *agentManagementSettingRepoStub) SetMultiple(context.Context, map[string]string) error {
	panic("unexpected SetMultiple call")
}

func (r *agentManagementSettingRepoStub) GetAll(context.Context) (map[string]string, error) {
	panic("unexpected GetAll call")
}

func (r *agentManagementSettingRepoStub) Delete(context.Context, string) error {
...
func mustFindAgentManagementDelegation(t *testing.T, repo *agentManagementRepoStub, managerID int64, childID int64, groupID int64) agentGroupDelegationRecord {
	t.Helper()
	for _, delegation := range repo.groupDelegations {
		if delegation.managerID == managerID && delegation.childID == childID && delegation.groupID == groupID {
			return delegation
		}
	}
	t.Fatalf("delegation not found: manager=%d child=%d group=%d", managerID, childID, groupID)
	return agentGroupDelegationRecord{}
}
...
func (r *agentManagementRepoStub) GetAgentIncomeTotals(_ context.Context, agentIDs []int64) (map[int64]float64, error) {
	out := make(map[int64]float64, len(agentIDs))
	for _, id := range agentIDs {
		if r.agentIncomeTotals != nil {
			out[id] = r.agentIncomeTotals[id]
		}
	}
	return out, nil
}
...
func TestAgentManagementUpgradeRules(t *testing.T) {
	...
	got, err = svc.UpgradeDirectUser(context.Background(), rootID, anotherUserUnderAdminID, AgentUpgradeInput{TargetRole: RoleEnterprise, PoolConcurrency: 50, PoolRPM: 500})
	require.NoError(t, err)
	require.Equal(t, RoleEnterprise, got.Role)

	got, err = svc.UpgradeDirectUser(context.Background(), level1ID, userUnderLevel1ID, AgentUpgradeInput{TargetRole: RoleEnterprise, PoolConcurrency: 50, PoolRPM: 500})
	require.NoError(t, err)
	require.Equal(t, RoleEnterprise, got.Role)
}
...
func TestAgentManagementAllocationCannotExceedRemaining(t *testing.T) {
	...
	summary, err := svc.UpdateAllocation(context.Background(), managerID, childID, AllocationUpdate{AllocatedConcurrency: 39, AllocatedRPM: 399})
	require.NoError(t, err)
	require.Equal(t, 100, summary.TotalConcurrency)
	require.Equal(t, 99, summary.AllocatedConcurrency)
	require.Equal(t, 1, summary.RemainingConcurrency)
	require.Equal(t, 39, repo.users[childID].Concurrency)
	require.Equal(t, 399, repo.users[childID].RPMLimit)

	_, err = svc.UpdateAllocation(context.Background(), managerID, childID, AllocationUpdate{AllocatedConcurrency: 40, AllocatedRPM: 400})
...
func TestAgentManagementAdminAgentTreeShowsAgentsUsersEnterprisesAndEmployees(t *testing.T) {
	rootID := int64(1)
	agentID := int64(2)
	directUserID := int64(3)
	enterpriseID := int64(4)
	employeeID := int64(5)
	adminDirectUserID := int64(6)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: agentID, Role: RoleAgentLevel1, ParentUserID: &rootID, Email: "agent@example.test", Status: StatusActive},
		&User{ID: directUserID, Role: RoleUser, ParentUserID: &agentID, Email: "user@example.test", Concurrency: 3, RPMLimit: 30, Status: StatusActive},
		&User{ID: enterpriseID, Role: RoleEnterprise, ParentUserID: &agentID, Email: "enterprise@example.test", Status: StatusActive},
		&User{ID: employeeID, Role: RoleEmployee, ParentUserID: &enterpriseID, Email: "employee@example.test", Concurrency: 2, RPMLimit: 20, Status: StatusActive},
		&User{ID: adminDirectUserID, Role: RoleUser, ParentUserID: &rootID, Email: "admin-user@example.test", Status: StatusActive},
	)
	repo.agentProfiles = map[int64]AgentProfile{
		agentID: {UserID: agentID, PoolConcurrency: 100, PoolRPM: 1000},
	}
	repo.enterpriseProfiles = map[int64]EnterpriseProfile{
		enterpriseID: {UserID: enterpriseID, PoolConcurrency: 10, PoolRPM: 100},
	}
	repo.agentIncomeTotals = map[int64]float64{agentID: 12.34}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)

	result, err := svc.GetAdminAgentTree(context.Background(), rootID)
	require.NoError(t, err)
	require.Len(t, result.Items, 1)
	require.Equal(t, agentID, result.Items[0].Agent.ID)
	require.NotNil(t, result.Items[0].Agent.AgentProfile)
	require.Equal(t, 100, result.Items[0].Agent.AgentProfile.PoolConcurrency)
	require.InDelta(t, 12.34, result.Items[0].Agent.AgentIncome, 1e-12)
	require.Len(t, result.Items[0].Users, 1)
	require.Equal(t, directUserID, result.Items[0].Users[0].ID)
	require.Len(t, result.Items[0].Enterprises, 1)
	require.Equal(t, enterpriseID, result.Items[0].Enterprises[0].Enterprise.ID)
	require.NotNil(t, result.Items[0].Enterprises[0].Enterprise.EnterpriseProfile)
	require.Equal(t, 10, result.Items[0].Enterprises[0].Enterprise.EnterpriseProfile.PoolConcurrency)
	require.Len(t, result.Items[0].Enterprises[0].Employees, 1)
	require.Equal(t, employeeID, result.Items[0].Enterprises[0].Employees[0].ID)
	require.NotEqual(t, adminDirectUserID, result.Items[0].Users[0].ID)
}

func TestAgentManagementAdminAgentTreeRejectsAgents(t *testing.T) {
	rootID := int64(1)
	agentID := int64(2)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
...
func TestAgentManagementAgentPoolControlsManagerCapacity(t *testing.T) {
	...
	repo.enterpriseProfiles = map[int64]EnterpriseProfile{
		otherUserID: {UserID: otherUserID, PoolConcurrency: 30, PoolRPM: 300},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)
	...
	require.NoError(t, err)
	require.Equal(t, 100, summary.TotalConcurrency)
	require.Equal(t, 70, summary.AllocatedConcurrency)
	require.Equal(t, 30, summary.RemainingConcurrency)
	require.Equal(t, 30, repo.users[managerID].Concurrency)
	require.Equal(t, 300, repo.users[managerID].RPMLimit)
	...
	_, err = svc.UpdateAllocation(context.Background(), managerID, childID, AllocationUpdate{AllocatedConcurrency: 70, AllocatedRPM: 700})
	require.ErrorIs(t, err, ErrAgentManagementAllocationExceeded)
}

func TestAgentManagementAgentPoolRejectsAllocationThatExhaustsManagerRPM(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	childID := int64(10)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: managerID, Role: RoleAgentLevel1, ParentUserID: &rootID, Concurrency: 10, RPMLimit: 100, Status: StatusActive},
		&User{ID: childID, Role: RoleUser, ParentUserID: &managerID, Concurrency: 1, RPMLimit: 10, Status: StatusActive},
	)
	repo.agentProfiles = map[int64]AgentProfile{
	...
	_, err := svc.UpdateAllocation(context.Background(), managerID, childID, AllocationUpdate{AllocatedConcurrency: 5, AllocatedRPM: 100})

	require.ErrorIs(t, err, ErrAgentManagementAllocationExceeded)
	require.Equal(t, 100, repo.users[managerID].RPMLimit)
	require.Equal(t, 10, repo.users[childID].RPMLimit)
}

func TestAgentManagementFiniteAgentCannotAllocateUnlimitedRPMToDirectUser(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	childID := int64(10)
	repo := newAgentManagementRepoStub(
	...
	_, err := svc.UpdateAllocation(context.Background(), managerID, childID, AllocationUpdate{AllocatedConcurrency: 1, AllocatedRPM: 0})
...
func TestAgentManagementUpdateDirectEnterprisePoolRejectsReclaimThatLeavesNoCapacity(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	enterpriseID := int64(3)
	employeeID := int64(4)
	...
	require.Equal(t, 20, details.RequestedConcurrency)
	require.Equal(t, 200, details.AllocatedRPM)
	require.Equal(t, 200, details.RequestedRPM)
...
func TestAgentManagementUpdateDirectAgentPoolUpdatesProfileAndEffectiveQuota(t *testing.T) {
	...
	summary, err := svc.UpdateAllocation(context.Background(), rootID, childAgentID, AllocationUpdate{
		Concurrency: intPtr(50),
		RPM:         intPtr(500),
	})
	require.NoError(t, err)
	...
	require.Equal(t, 50, repo.users[childAgentID].Concurrency)
	require.Equal(t, 500, repo.users[childAgentID].RPMLimit)
	require.Equal(t, 50, summary.AllocatedConcurrency)
	require.Equal(t, 500, summary.AllocatedRPM)
	require.Contains(t, invalidator.userIDs, childAgentID)
}

func TestAgentManagementUpdateDirectAgentPoolRejectsReclaimThatLeavesNoCapacity(t *testing.T) {
	rootID := int64(1)
	childAgentID := int64(3)
	...
	_, err := svc.UpdateAllocation(context.Background(), rootID, childAgentID, AllocationUpdate{
		Concurrency: intPtr(20),
		RPM:         intPtr(200),
	})
	require.ErrorIs(t, err, ErrAgentManagementPoolReclaimExceeded)
...
func TestAgentManagementFiniteAgentCannotAllocateUnlimitedPool(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	childEnterpriseID := int64(3)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: managerID, Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: childEnterpriseID, Role: RoleEnterprise, ParentUserID: &managerID, Status: StatusActive},
	)
	repo.agentProfiles = map[int64]AgentProfile{
		managerID: {UserID: managerID, PoolConcurrency: 100, PoolRPM: 1000},
	}
	repo.enterpriseProfiles = map[int64]EnterpriseProfile{
		childEnterpriseID: {UserID: childEnterpriseID, PoolConcurrency: 40, PoolRPM: 400},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)

	_, err := svc.UpdateAllocation(context.Background(), managerID, childEnterpriseID, AllocationUpdate{
		Concurrency: intPtr(0),
		RPM:         intPtr(0),
	})
	require.ErrorIs(t, err, ErrAgentManagementInvalidAllocation)
}

func TestAgentManagementAgentWithUnlimitedRPMStillRequiresPositiveConcurrencyPool(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	childEnterpriseID := int64(3)
	repo := newAgentManagementRepoStub(
	...
	repo.agentProfiles = map[int64]AgentProfile{
		managerID: {UserID: managerID, PoolConcurrency: 10, PoolRPM: 0},
	}
	repo.enterpriseProfiles = map[int64]EnterpriseProfile{
		childEnterpriseID: {UserID: childEnterpriseID, PoolConcurrency: 40, PoolRPM: 400},
	}
	...
	require.NoError(t, err)
	require.Equal(t, 5, repo.enterpriseProfiles[childEnterpriseID].PoolConcurrency)
	require.Equal(t, 0, repo.enterpriseProfiles[childEnterpriseID].PoolRPM)
}
...
func TestAgentManagementUpdateInviteDefaults(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 3, profile.InviteDefaultConcurrency)
	require.Equal(t, 30, profile.InviteDefaultRPM)
	require.Equal(t, 3, repo.agentProfiles[managerID].InviteDefaultConcurrency)
	require.Equal(t, 30, repo.agentProfiles[managerID].InviteDefaultRPM)
...
func TestAgentManagementAdminInviteDefaultsSyncGlobalSettings(t *testing.T) {
	adminID := int64(1)
	repo := newAgentManagementRepoStub(
		&User{ID: adminID, Role: RoleAdmin, Status: StatusActive},
	)
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	settingRepo := &agentManagementSettingRepoStub{values: map[string]string{
		SettingKeyDefaultConcurrency:  "7",
		SettingKeyDefaultUserRPMLimit: "70",
	}}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)
	...
	require.Equal(t, 7, summary.InviteDefaults.InviteDefaultConcurrency)
	require.Equal(t, 70, summary.InviteDefaults.InviteDefaultRPM)

	profile, err := svc.UpdateInviteDefaults(context.Background(), adminID, AgentInviteDefaultsUpdate{
		InviteDefaultConcurrency: 8,
		InviteDefaultRPM:         80,
	})
	require.NoError(t, err)
	require.Equal(t, adminID, profile.UserID)
	require.Equal(t, 8, profile.InviteDefaultConcurrency)
	require.Equal(t, 80, profile.InviteDefaultRPM)
	require.Equal(t, "8", settingRepo.sets[SettingKeyDefaultConcurrency])
	require.Equal(t, "80", settingRepo.sets[SettingKeyDefaultUserRPMLimit])
	require.Empty(t, repo.agentProfiles)
}

func TestAgentManagementFiniteAgentCannotAllocateWhenExistingChildRPMIsUnlimited(t *testing.T) {
	rootID := int64(1)
	managerID := int64(2)
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: managerID, Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: unlimitedChildID, Role: RoleUser, ParentUserID: &managerID, Concurrency: 1, RPMLimit: 0, Status: StatusActive},
...
func TestAgentManagementCannotReclaimAgentPoolToFiniteWhenChildUsageIsUnlimited(t *testing.T) {
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: childAgentID, Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: grandchildID, Role: RoleUser, ParentUserID: &childAgentID, Concurrency: 1, RPMLimit: 0, Status: StatusActive},
	)
	repo.agentProfiles = map[int64]AgentProfile{
		childAgentID: {UserID: childAgentID, PoolConcurrency: 100, PoolRPM: 0},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)
...
func TestAgentManagementDeleteRules(t *testing.T) {
	...
	enterpriseUnderDeletedAgentID := int64(22)
	users := []*User{
		...
		{ID: enterpriseUnderDeletedAgentID, Role: RoleEnterprise, ParentUserID: &level1UnderAdminID},
	}
	repo := newAgentManagementRepoStub(users...)
	repo.groupDelegations = []agentGroupDelegationRecord{
		...
		{managerID: level1UnderAdminID, childID: enterpriseUnderDeletedAgentID, groupID: 22, rateMultiplier: 2.7, canDelegate: true},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	invalidator := &agentManagementAuthInvalidatorStub{}
	...
	require.Equal(t, rootID, *repo.users[enterpriseUnderDeletedAgentID].ParentUserID)
	require.Equal(t, RoleEnterprise, repo.users[enterpriseUnderDeletedAgentID].Role)
	require.Equal(t, []agentGroupDelegationRecord{
		...
		{managerID: rootID, childID: enterpriseUnderDeletedAgentID, groupID: 22, rateMultiplier: 2.7, canDelegate: true},
	}, repo.groupDelegations)
	...
	require.Contains(t, invalidator.userIDs, enterpriseUnderDeletedAgentID)
}
...
func TestAgentManagementLevel1DeletingDirectUserMovesUserToRoot(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	directUserID := int64(3)
	repo := newAgentManagementRepoStub(
	...
	require.NoError(t, svc.DeleteDirectChild(context.Background(), level1ID, directUserID))

	require.Equal(t, rootID, *repo.users[directUserID].ParentUserID)
	require.Equal(t, RoleUser, repo.users[directUserID].Role)
	require.Equal(t, 60, repo.users[level1ID].Concurrency)
	require.Equal(t, 600, repo.users[level1ID].RPMLimit)
	require.Empty(t, userRepo.deletedUserIDs)
	require.Contains(t, invalidator.userIDs, directUserID)
}
...
func TestAgentManagementResolveInvitationParent(t *testing.T) {
	...
	got, err = svc.ResolveInvitationParent(context.Background(), ordinaryUnderLevel1ID)
	require.NoError(t, err)
	require.NotNil(t, got)
	require.Equal(t, level1ID, *got)
...
func TestChildGroupDelegationOptionsShowsAssignedStateForDirectChild(t *testing.T) {
	...
	repo.groupDelegations = []agentGroupDelegationRecord{
		...
		{managerID: level1ID, childID: childID, groupID: 20, rateMultiplier: 2.4, canDelegate: false},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	groupRepo := newAgentManagementGroupRepoStub(
		Group{ID: 10, Name: "public", RateMultiplier: 1.2, Status: StatusActive},
	...
	options, err := svc.ListChildGroupDelegationOptions(context.Background(), level1ID, childID)
	require.NoError(t, err)
...
func TestAdminInviteGroupDefaultOptionsShowExclusiveGroups(t *testing.T) {
	rootID := int64(1)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
	)
	repo.inviteGroupDefaults = []agentGroupDelegationRecord{
		{managerID: rootID, groupID: 20, rateMultiplier: 2.4},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	groupRepo := newAgentManagementGroupRepoStub(
		Group{ID: 10, Name: "public", RateMultiplier: 1.2, Status: StatusActive},
	...
	options, err := svc.ListInviteGroupDefaultOptions(context.Background(), rootID)
	require.NoError(t, err)

	require.Len(t, options, 1)
	require.Equal(t, int64(20), options[0].Group.ID)
	require.Equal(t, 0.3, options[0].EffectiveRate)
	require.True(t, options[0].CanDelegate)
	require.True(t, options[0].Assigned)
	require.Equal(t, 2.4, options[0].ChildRateMultiplier)
}
...
func TestAdminCanSetInviteGroupDefault(t *testing.T) {
	...
	require.NoError(t, svc.SetInviteGroupDefault(context.Background(), rootID, 20, AgentInviteGroupDefaultInput{RateMultiplier: 2.1}))
	require.Equal(t, []agentGroupDelegationRecord{{managerID: rootID, groupID: 20, rateMultiplier: 2.1}}, repo.upsertInviteDefaults)
}

func TestAdminCanDelegateExclusiveGroupAtZeroCost(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	groupID := int64(20)
	repo := newAgentManagementRepoStub(
	...
	err := svc.SetChildGroupDelegation(context.Background(), rootID, level1ID, groupID, ChildGroupDelegationInput{RateMultiplier: 0, CanDelegate: true})

	require.NoError(t, err)
	require.Equal(t, 0.0, mustFindAgentManagementDelegation(t, repo, rootID, level1ID, groupID).rateMultiplier)
	require.True(t, mustFindAgentManagementDelegation(t, repo, rootID, level1ID, groupID).canDelegate)
}

func TestAgentCanDelegateExclusiveGroupAtZeroWhenDelegatedCostIsZero(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	directUserID := int64(3)
	groupID := int64(20)
	...
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: level1ID, groupID: groupID, rateMultiplier: 0, canDelegate: true},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	groupRepo := newAgentManagementGroupRepoStub(Group{ID: groupID, Name: "exclusive", IsExclusive: true, Status: StatusActive})
	svc := NewAgentManagementService(repo, userRepo, groupRepo, nil)

	err := svc.SetChildGroupDelegation(context.Background(), level1ID, directUserID, groupID, ChildGroupDelegationInput{RateMultiplier: 0, CanDelegate: false})

	require.NoError(t, err)
	require.Equal(t, 0.0, mustFindAgentManagementDelegation(t, repo, level1ID, directUserID, groupID).rateMultiplier)
}
...
func TestApplyAdminInviteGroupDefaultsToRegisteredChild(t *testing.T) {
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: childID, Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
	)
	repo.inviteGroupDefaults = []agentGroupDelegationRecord{
	...
	require.NoError(t, svc.ApplyInviteGroupDefaultsToChild(context.Background(), rootID, childID))

	require.Equal(t, []agentGroupDelegationRecord{
		{managerID: rootID, childID: childID, groupID: groupID, rateMultiplier: 2.4, canDelegate: false},
	}, repo.groupDelegations)
	require.Equal(t, []int64{groupID}, repo.users[childID].AllowedGroups)
	require.Equal(t, 2.4, groupRateRepo.rates[childID][groupID])
}
...
func TestDelegateExclusiveGroupRequiresManagerAccess(t *testing.T) {
	...
	err := svc.SetChildGroupDelegation(context.Background(), level1ID, directUserID, 20, ChildGroupDelegationInput{RateMultiplier: 1.8, CanDelegate: false})
	require.ErrorIs(t, err, ErrAgentManagementForbidden)
	...
	require.NoError(t, svc.SetChildGroupDelegation(context.Background(), level1ID, directUserID, 20, ChildGroupDelegationInput{RateMultiplier: 1.8, CanDelegate: false}))
...
func TestAgentCannotDelegateExclusiveGroupAtZeroWhenDelegatedCostIsPositive(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	directUserID := int64(3)
	groupID := int64(20)
...
func TestAdminRaisingEnterpriseDelegatedRateRaisesEmployeeRateFloor(t *testing.T) {
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: enterpriseID, Role: RoleEnterprise, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: employeeID, Role: RoleEmployee, ParentUserID: &enterpriseID, Status: StatusActive},
	)
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: enterpriseID, groupID: groupID, rateMultiplier: 1.5, canDelegate: true},
		{managerID: enterpriseID, childID: employeeID, groupID: groupID, rateMultiplier: 1.6, canDelegate: false},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	invalidator := &agentManagementAuthInvalidatorStub{}
	groupRepo := newAgentManagementGroupRepoStub(Group{ID: groupID, Name: "exclusive", IsExclusive: true, Status: StatusActive})
	...
	require.NoError(t, svc.SetChildGroupDelegation(context.Background(), rootID, enterpriseID, groupID, ChildGroupDelegationInput{RateMultiplier: 2.0, CanDelegate: true}))
...
func TestSetChildGroupDelegationsBatchAppliesSelectedGroups(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	directUserID := int64(3)
	repo := newAgentManagementRepoStub(
	...
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: level1ID, groupID: 20, rateMultiplier: 1.5, canDelegate: true},
		{managerID: rootID, childID: level1ID, groupID: 30, rateMultiplier: 1.6, canDelegate: true},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	groupRepo := newAgentManagementGroupRepoStub(
	...
	require.NoError(t, svc.SetChildGroupDelegationsBatch(context.Background(), level1ID, directUserID, ChildGroupDelegationBatchInput{
		GroupIDs:       []int64{20, 20, 30},
		RateMultiplier: 2.0,
		CanDelegate:    false,
	}))

	require.NotNil(t, mustFindAgentManagementDelegation(t, repo, level1ID, directUserID, 20))
...
func TestSetDirectChildrenGroupDelegationsBatchDeploysMissingGroupsOnlyForKind(t *testing.T) {
	rootID := int64(1)
	userAID := int64(2)
	userBID := int64(3)
	agentID := int64(4)
	selectedChildrenOnly := false
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
...
func TestListDirectChildrenWithGroupDelegationReturnsOnlyAssignedChildrenOfKind(t *testing.T) {
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: userAID, Email: "alpha@example.test", Username: "alpha", Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: userBID, Email: "beta@example.test", Username: "beta", Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: agentID, Email: "agent@example.test", Username: "agent", Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
	)
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: userAID, groupID: groupID, rateMultiplier: 1.2, canDelegate: false},
		{managerID: rootID, childID: agentID, groupID: groupID, rateMultiplier: 1.4, canDelegate: true},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)

	result, err := svc.ListDirectChildrenWithGroupDelegation(context.Background(), rootID, DirectChildKindUsers, DirectChildrenGroupQuery{
		GroupID: groupID,
		Search:  "alp",
	...
	require.Len(t, result.Users, 1)
	require.Equal(t, userAID, result.Users[0].ID)
	require.Equal(t, 1.2, result.Users[0].GroupRates[groupID])
	require.Equal(t, int64(1), result.Pagination.Total)
}

func TestListDirectChildrenWithoutGroupDelegationReturnsChildrenMissingAnySelectedGroup(t *testing.T) {
	rootID := int64(1)
	userAllGroupsID := int64(2)
	userMissingOneID := int64(3)
	userMissingAllID := int64(4)
	agentID := int64(5)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: userAllGroupsID, Email: "alpha@example.test", Username: "alpha", Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: userMissingOneID, Email: "beta@example.test", Username: "beta", Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: userMissingAllID, Email: "gamma@example.test", Username: "gamma", Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
		&User{ID: agentID, Email: "agent@example.test", Username: "agent", Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
	)
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: userAllGroupsID, groupID: 20, rateMultiplier: 1.2, canDelegate: false},
		{managerID: rootID, childID: userAllGroupsID, groupID: 30, rateMultiplier: 1.3, canDelegate: false},
		{managerID: rootID, childID: userMissingOneID, groupID: 20, rateMultiplier: 1.2, canDelegate: false},
		{managerID: rootID, childID: agentID, groupID: 20, rateMultiplier: 1.4, canDelegate: true},
	}
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	svc := NewAgentManagementService(repo, userRepo, nil, nil)

	result, err := svc.ListDirectChildrenWithoutGroupDelegation(context.Background(), rootID, DirectChildKindUsers, DirectChildrenGroupQuery{
	...
	require.Equal(t, []int64{userMissingOneID, userMissingAllID}, []int64{result.Users[0].ID, result.Users[1].ID})
	require.Equal(t, int64(2), result.Pagination.Total)
}

func TestUpdateDirectChildrenExistingGroupDelegationsOnlyUpdatesAssignedChildren(t *testing.T) {
	rootID := int64(1)
	userAID := int64(2)
	userBID := int64(3)
	groupID := int64(20)
	...
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: userAID, Role: RoleUser, ParentUserID: &rootID, AllowedGroups: []int64{groupID}, Status: StatusActive},
		&User{ID: userBID, Role: RoleUser, ParentUserID: &rootID, Status: StatusActive},
	)
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: userAID, groupID: groupID, rateMultiplier: 1.2, canDelegate: false},
	...
	result, err := svc.UpdateDirectChildrenExistingGroupDelegations(context.Background(), rootID, DirectChildKindUsers, DirectChildrenGroupDelegationUpdateInput{
		GroupID:        groupID,
		All:            true,
		RateMultiplier: &rate,
		CanDelegate:    &canDelegate,
	...
	require.Equal(t, DirectChildKindUsers, result.Kind)
	require.Equal(t, 1, result.UpdatedChildren)
	require.Equal(t, 1, result.SkippedChildren)
	require.Equal(t, 2.0, mustFindAgentManagementDelegation(t, repo, rootID, userAID, groupID).rateMultiplier)
	require.True(t, mustFindAgentManagementDelegation(t, repo, rootID, userAID, groupID).canDelegate)
	missing, err := repo.GetGroupDelegation(context.Background(), rootID, userBID, groupID)
	require.NoError(t, err)
	require.Nil(t, missing)
	require.Empty(t, repo.users[userBID].AllowedGroups)
}

func TestUpdateDirectChildrenExistingGroupDelegationsTriggersExistingRateFloorCascade(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	directUserID := int64(3)
	groupID := int64(20)
	...
	result, err := svc.UpdateDirectChildrenExistingGroupDelegations(context.Background(), rootID, DirectChildKindAgents, DirectChildrenGroupDelegationUpdateInput{
		GroupID:        groupID,
		ChildIDs:       []int64{level1ID},
		RateMultiplier: &rate,
	})
	require.NoError(t, err)

	require.Equal(t, 1, result.UpdatedChildren)
	require.Equal(t, 2.0, mustFindAgentManagementDelegation(t, repo, rootID, level1ID, groupID).rateMultiplier)
	require.Equal(t, 2.0, mustFindAgentManagementDelegation(t, repo, level1ID, directUserID, groupID).rateMultiplier)
	require.Equal(t, []struct {
		agentID     int64
		groupID     int64
		minimumRate float64
...
func TestUpdateDirectChildrenExistingGroupDelegationsRaisesEnterpriseEmployeeFloor(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	enterpriseID := int64(3)
	employeeID := int64(4)
	...
	result, err := svc.UpdateDirectChildrenExistingGroupDelegations(context.Background(), level1ID, DirectChildKindEnterprises, DirectChildrenGroupDelegationUpdateInput{
		GroupID:        groupID,
		ChildIDs:       []int64{enterpriseID},
		RateMultiplier: &rate,
	})
	require.NoError(t, err)
...
func TestRemoveDirectChildrenGroupDelegationsBatchRemovesAssignedChildrenAndCascades(t *testing.T) {
	rootID := int64(1)
	level1ID := int64(2)
	enterpriseID := int64(3)
	employeeID := int64(4)
	missingEnterpriseID := int64(5)
	groupID := int64(20)
	repo := newAgentManagementRepoStub(
		&User{ID: rootID, Role: RoleAdmin, Status: StatusActive},
		&User{ID: level1ID, Role: RoleAgentLevel1, ParentUserID: &rootID, Status: StatusActive},
		...
		&User{ID: missingEnterpriseID, Role: RoleEnterprise, ParentUserID: &level1ID, Status: StatusActive},
	)
	repo.groupDelegations = []agentGroupDelegationRecord{
		{managerID: rootID, childID: level1ID, groupID: groupID, rateMultiplier: 1.5, canDelegate: true},
		{managerID: level1ID, childID: enterpriseID, groupID: groupID, rateMultiplier: 1.6, canDelegate: true},
	...
	userRepo := &agentManagementUserRepoStub{users: repo.users}
	rateRepo := &agentManagementUserGroupRateRepoStub{rates: map[int64]map[int64]float64{
		enterpriseID: {groupID: 1.6},
		employeeID:   {groupID: 1.7},
	}}
	invalidator := &agentManagementAuthInvalidatorStub{}
	svc := NewAgentManagementService(repo, userRepo, nil, invalidator)
	svc.SetUserGroupRateRepository(rateRepo)

	result, err := svc.RemoveDirectChildrenGroupDelegationsBatch(context.Background(), level1ID, DirectChildKindEnterprises, DirectChildrenGroupDelegationReclaimInput{
		GroupID: groupID,
		All:     true,
	})
	require.NoError(t, err)

	require.Equal(t, DirectChildKindEnterprises, result.Kind)
	require.Equal(t, 1, result.RemovedChildren)
	require.Equal(t, 1, result.SkippedChildren)
	enterpriseDelegation, err := repo.GetGroupDelegation(context.Background(), level1ID, enterpriseID, groupID)
	require.NoError(t, err)
	require.Nil(t, enterpriseDelegation)
	employeeDelegation, err := repo.GetGroupDelegation(context.Background(), enterpriseID, employeeID, groupID)
	require.NoError(t, err)
	require.Nil(t, employeeDelegation)
	require.Empty(t, repo.users[enterpriseID].AllowedGroups)
	require.Empty(t, repo.users[employeeID].AllowedGroups)
	require.NotContains(t, rateRepo.rates[enterpriseID], groupID)
	require.NotContains(t, rateRepo.rates[employeeID], groupID)
	require.ElementsMatch(t, []int64{enterpriseID, employeeID}, invalidator.userIDs)
}
...
func TestSetInviteGroupDefaultsBatchAppliesAllDelegableGroups(t *testing.T) {
	...
	groupRepo := newAgentManagementGroupRepoStub(
		Group{ID: 20, Name: "exclusive-a", IsExclusive: true, Status: StatusActive},
		Group{ID: 30, Name: "exclusive-b", IsExclusive: true, Status: StatusActive},
		Group{ID: 40, Name: "public", IsExclusive: false, Status: StatusActive},
	)
	svc := NewAgentManagementService(repo, userRepo, groupRepo, nil)

	require.NoError(t, svc.SetInviteGroupDefaultsBatch(context.Background(), level1ID, AgentInviteGroupDefaultBatchInput{
		All:            true,
		RateMultiplier: 2.0,
	}))

	require.ElementsMatch(t, []agentGroupDelegationRecord{
		{managerID: level1ID, groupID: 20, rateMultiplier: 2.0},
		{managerID: level1ID, groupID: 30, rateMultiplier: 2.0},
	}, repo.upsertInviteDefaults)
}
