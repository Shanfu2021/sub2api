...
func TestAuthService_Register_IPLimitRejectsDuplicateSignupIP(t *testing.T) {
	repo := &userRepoStub{existsBySignupIP: true}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled:        "true",
...
func TestAuthService_Register_IPLimitDisabledAllowsSignup(t *testing.T) {
	repo := &userRepoStub{nextID: 6, existsBySignupIP: true}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled:        "true",
