...
func prioritizeOpenAIStickyCandidateScores(
	candidates []openAIAccountCandidateScore,
	req OpenAIAccountScheduleRequest,
	...
	for _, stickyID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		for i, candidate := range candidates {
			if candidate.account != nil && candidate.account.ID == stickyID {
				ordered := append([]openAIAccountCandidateScore{candidate}, candidates[:i]...)
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	if req.StrictPriority {
		return s.buildOpenAIStrictPrioritySelectionOrder(req, plan)
	}
	...
	buildSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		if ordered, ok := prioritizeOpenAIStickyCandidateScores(ranked, req); ok {
			return ordered
		}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIStrictPrioritySelectionOrder(
	req OpenAIAccountScheduleRequest,
	plan openAIAccountLoadPlan,
) []openAIAccountCandidateScore {
	pool := append([]openAIAccountCandidateScore(nil), plan.candidates...)
	...
	for _, candidate := range pool {
		...
		priorityBuckets[priority] = append(priorityBuckets[priority], candidate)
	}
	...
	// strict_priority never spills into a higher-priority bucket merely because
	// every account in the lowest eligible bucket is currently busy. The caller
	// retries this bucket with fresh load and then builds a wait plan from it.
	bucket := sortOpenAIStrictPriorityBucketCandidates(priorityBuckets[priorities[0]])
	if ordered, ok := prioritizeOpenAIStickyCandidateScores(bucket, req); ok {
		bucket = ordered
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	if req.SubscriptionPriority && !req.StrictPriority {
