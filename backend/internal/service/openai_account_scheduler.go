...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if selection != nil && selection.Account != nil && !req.StrictPriority {
...
func (s *defaultOpenAIAccountScheduler) selectStrictPriorityFromOrder(
	...
	balancedReq := req
	balancedReq.SessionHash = ""
	balancedReq.PreviousResponseID = ""
	for _, priority := range priorities {
		bucket := buildOpenAIWeightedSelectionOrder(priorityBuckets[priority], balancedReq)
		if req.RequiredImageCapability != "" {
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	if paused, decision := shouldAutoPauseOpenAIAccountByQuota(ctx, account); paused {
		if s != nil && s.service != nil {
			s.service.persistOpenAIQuotaAutoPause(ctx, account, decision)
		}
		return false
	}
...
func (s *OpenAIGatewayService) SelectResponsesAccountWithScheduler(
	ctx context.Context,
	groupID *int64,
	previousResponseID string,
	sessionHash string,
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, OpenAIEndpointCapabilityChatCompletions, "", requireCompact, true)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, sessionHash, requestedModel, effectiveExcludedIDs, requireCompact, requiredCapability, requiredImageCapability != "")
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		RequireResponsesAPI:     requireResponsesAPI,
