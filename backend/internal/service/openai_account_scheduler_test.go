...
type schedulerSequencedConcurrencyCache struct {
	ConcurrencyCache
	mu           sync.Mutex
...
func (c *schedulerSequencedConcurrencyCache) AcquireAccountSlot(context.Context, int64, int, string) (bool, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.acquireCalls++
	return c.acquireCalls > c.acquireAfter, nil
}

func (c *schedulerSequencedConcurrencyCache) ReleaseAccountSlot(context.Context, int64, string) error {
	return nil
}

func (c *schedulerSequencedConcurrencyCache) GetAccountsLoadBatch(_ context.Context, accounts []AccountWithConcurrency) (map[int64]*AccountLoadInfo, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	for _, account := range accounts {
		if index >= 0 {
			if load := c.loadMaps[index][account.ID]; load != nil {
				cloned := *load
				out[account.ID] = &cloned
				continue
			}
		}
...
func (c *schedulerSequencedConcurrencyCache) snapshotCalls() (loadCalls int, acquireCalls int) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.loadCalls, c.acquireCalls
}
...
func TestSortOpenAIStrictPriorityBucketCandidatesOrdersByCostThenLoad(t *testing.T) {
	now := time.Now()
	...
	candidates := []openAIAccountCandidateScore{
		{
			account:   &Account{ID: 37101, Priority: 1, RateMultiplier: &highRate},
			loadInfo:  &AccountLoadInfo{AccountID: 37101, LoadRate: 0, WaitingCount: 0},
			score:     10,
			errorRate: 0,
		},
		{
			account:   &Account{ID: 37102, Priority: 1, RateMultiplier: &lowRate, LastUsedAt: &now},
			loadInfo:  &AccountLoadInfo{AccountID: 37102, LoadRate: 80, WaitingCount: 0},
			score:     1,
			errorRate: 0,
		},
		{
			account:   &Account{ID: 37103, Priority: 1, RateMultiplier: &sameLowRate, LastUsedAt: &older},
			loadInfo:  &AccountLoadInfo{AccountID: 37103, LoadRate: 20, WaitingCount: 0},
			score:     1,
			errorRate: 0,
		},
	}

	ordered := sortOpenAIStrictPriorityBucketCandidates(candidates)

	require.Equal(t, []int64{37103, 37102, 37101}, openAIAccountCandidateIDs(ordered))
	require.Equal(t, []int64{37101, 37102, 37103}, openAIAccountCandidateIDs(candidates), "input order must not be mutated")
}

func TestDefaultOpenAIAccountScheduler_BuildStrictPrioritySelectionOrderComposesStickyAndIgnoresTopK(t *testing.T) {
	highRate := 1.4
	lowRate := 0.7
	lowerPriorityRate := 0.1
	candidates := []openAIAccountCandidateScore{
		{
			account:  &Account{ID: 37111, Priority: 1, RateMultiplier: &highRate},
			loadInfo: &AccountLoadInfo{AccountID: 37111, LoadRate: 30},
		},
		{
			account:  &Account{ID: 37112, Priority: 1, RateMultiplier: &lowRate},
			loadInfo: &AccountLoadInfo{AccountID: 37112, LoadRate: 10},
		},
		{
			account:  &Account{ID: 37113, Priority: 2, RateMultiplier: &lowerPriorityRate},
			loadInfo: &AccountLoadInfo{AccountID: 37113, LoadRate: 0},
		},
	}
	...
	ordered := scheduler.buildOpenAISelectionOrder(OpenAIAccountScheduleRequest{
		StrictPriority:          true,
		StickyWeighted:          true,
		StickyAccountID:         37111,
		StickyPreviousAccountID: 37113,
	}, openAIAccountLoadPlan{
		candidates: candidates,
		topK:       1,
	})

	require.Equal(t, []int64{37111, 37112}, openAIAccountCandidateIDs(ordered))
}

func TestDefaultOpenAIAccountScheduler_SelectStrictPriorityPrefersLowerRateMultiplierWithinPriority(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10113)
	lowRate := 0.7
	highRate := 1.4
	...
	accounts := []Account{
		...
		},
		{
			...
			Concurrency:    1,
			Priority:       2,
			RateMultiplier: &lowerPriorityRate,
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo: schedulerTestOpenAIAccountRepo{accounts: accounts},
		...
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{
				...
				37102: {AccountID: 37102, LoadRate: 80, WaitingCount: 0},
				37103: {AccountID: 37103, LoadRate: 0, WaitingCount: 0},
			},
		}),
	}
...
func TestDefaultOpenAIAccountScheduler_SelectStrictPriorityWaitsForLowestEligibleBucket(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10117)
	accounts := []Account{
		{
	...
	svc := &OpenAIGatewayService{
		...
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{
				37501: {AccountID: 37501, LoadRate: 100},
				37502: {AccountID: 37502, LoadRate: 0},
			},
			acquireResults: map[int64]bool{
				37501: false,
				37502: true,
	...
	require.False(t, selection.Acquired)
	require.NotNil(t, selection.WaitPlan)
	require.Equal(t, int64(37501), selection.WaitPlan.AccountID)
}

func TestDefaultOpenAIAccountScheduler_SelectStrictPrioritySkipsExcludedFailedAccount(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10114)
	lowerRate := 0.5
	higherRate := 0.8
	lowerPriorityCheapRate := 0.1
	accounts := []Account{
		{
			ID:             37201,
			Platform:       PlatformOpenAI,
		...
		},
		{
			...
			Concurrency:    1,
			Priority:       2,
			RateMultiplier: &lowerPriorityCheapRate,
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo: schedulerTestOpenAIAccountRepo{accounts: accounts},
		...
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{
				37201: {AccountID: 37201, LoadRate: 0, WaitingCount: 0},
				37202: {AccountID: 37202, LoadRate: 0, WaitingCount: 0},
				37203: {AccountID: 37203, LoadRate: 0, WaitingCount: 0},
			},
		}),
	}
	scheduler := newDefaultOpenAIAccountScheduler(svc, nil)
	...
	selection, _, err := scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		RequestedModel: "gpt-5.1",
		StrictPriority: true,
		ExcludedIDs:    map[int64]struct{}{37201: {}},
	})
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
...
func TestDefaultOpenAIAccountScheduler_SelectStrictPriorityRetriesWithFreshLoad(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10115)
	lowerRate := 0.5
	higherRate := 0.8
	...
	concurrencyCache := &schedulerSequencedConcurrencyCache{
		loadMaps: []map[int64]*AccountLoadInfo{
			{
				37301: {AccountID: 37301, LoadRate: 10},
				37302: {AccountID: 37302, LoadRate: 20},
			},
			{
				37301: {AccountID: 37301, LoadRate: 5},
				37302: {AccountID: 37302, LoadRate: 25},
			},
		},
		acquireAfter: len(accounts),
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
		cfg:                &config.Config{},
	...
	loadCalls, acquireCalls := concurrencyCache.snapshotCalls()
	require.Equal(t, 2, loadCalls, "strict scheduling must use the fresh-load retry")
	require.Equal(t, len(accounts)+1, acquireCalls)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}

func TestDefaultOpenAIAccountScheduler_SelectStrictPriorityStickyEscapePreservesBinding(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10116)
	sessionHash := "strict-sticky-escape"
	stickyRate := 1.4
	fallbackRate := 0.7
	accounts := []Account{
		{
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{
			"openai:" + sessionHash: accounts[0].ID,
		},
	}
	stats := newOpenAIAccountRuntimeStats()
	slowTTFT := 20_000
	stats.report(accounts[0].ID, true, &slowTTFT)
	svc := &OpenAIGatewayService{
		accountRepo: schedulerTestOpenAIAccountRepo{accounts: accounts},
		cache:       cache,
		cfg:         &config.Config{},
	...
	require.Equal(t, accounts[0].ID, cache.sessionBindings["openai:"+sessionHash], "health escape must not replace the original sticky binding")
}
