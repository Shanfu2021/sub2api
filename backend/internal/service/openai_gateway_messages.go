...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	if finalResponse == nil {
		writeAnthropicError(c, http.StatusBadGateway, "api_error", "Service stream ended without a terminal response event")
	...
	if strings.TrimSpace(finalResponse.Status) == "failed" {
		...
		clientMessage := clientSafeUpstreamErrorMessage(http.StatusBadGateway)
		if isOpenAIContextWindowError(message, payload) {
			clientMessage = message
		}
		writeAnthropicError(c, http.StatusBadGateway, "api_error", clientMessage)
	...
	processDataLine := func(payload string) bool {
		...
		if isTerminalEvent {
			...
			if strings.TrimSpace(event.Type) == "response.failed" {
				...
				errStatus, errType, errMsg := http.StatusBadGateway, "api_error", clientMessage
				// 统一走语义状态推断 + body 归一化（与 /v1/responses 路径一致），
				// 使按错误码配置的透传规则可命中。
