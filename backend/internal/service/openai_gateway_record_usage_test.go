...
func TestAgentIncomeResolver_ZeroCostPreservesOwnerAndRates(t *testing.T) {
	groupID := int64(700)
	agentID := int64(800)
	enterpriseID := int64(900)
	userRepo := &openAIRecordUsageUserRepoStub{users: map[int64]*User{
		agentID:      {ID: agentID, Role: RoleAgentLevel1},
		enterpriseID: {ID: enterpriseID, Role: RoleEnterprise, ParentUserID: &agentID},
	}}
	rateRepo := &openAIUserGroupRateRepoStub{rates: map[int64]map[int64]float64{
		agentID: {groupID: 0.1},
	}}
	resolver := NewAgentIncomeResolver(userRepo, rateRepo)

	tests := []struct {
		name string
		user *User
	}{
		{name: "direct user", user: &User{ID: 1001, Role: RoleUser, ParentUserID: &agentID}},
		{name: "direct enterprise", user: &User{ID: enterpriseID, Role: RoleEnterprise, ParentUserID: &agentID}},
		{name: "enterprise employee", user: &User{ID: 1002, Role: RoleEmployee, ParentUserID: &enterpriseID}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			snapshot := resolver.Resolve(context.Background(), tt.user, &groupID, 0, 0.2, 1.0)
			require.NotNil(t, snapshot.AgentOwnerUserID)
			require.Equal(t, agentID, *snapshot.AgentOwnerUserID)
			require.InDelta(t, 0.2, snapshot.UserRateMultiplier, 1e-12)
			require.InDelta(t, 0.1, snapshot.AgentCostRateMultiplier, 1e-12)
			require.Zero(t, snapshot.AgentIncome)
		})
	}
}
