...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	keys := []string{
		...
		SettingKeyGmailVerificationBypassEnabled,
		SettingKeyForceEmailOnThirdPartySignup,
		SettingKeyRegistrationEmailSuffixWhitelist,
		SettingKeyRegistrationIPLimitEnabled,
		SettingKeyPromoCodeEnabled,
		SettingKeyPasswordResetEnabled,
		SettingKeyInvitationCodeEnabled,
		SettingKeyUserInviteRegistrationEnabled,
	...
	return &PublicSettings{
		...
		GmailVerificationBypassEnabled:   settings[SettingKeyGmailVerificationBypassEnabled] == "true",
		ForceEmailOnThirdPartySignup:     settings[SettingKeyForceEmailOnThirdPartySignup] == "true",
		RegistrationEmailSuffixWhitelist: registrationEmailSuffixWhitelist,
		RegistrationIPLimitEnabled:       settings[SettingKeyRegistrationIPLimitEnabled] == "true",
		PromoCodeEnabled:                 settings[SettingKeyPromoCodeEnabled] != "false", // 默认启用
		PasswordResetEnabled:             passwordResetEnabled,
		InvitationCodeEnabled:            settings[SettingKeyInvitationCodeEnabled] == "true",
		UserInviteRegistrationEnabled:    settings[SettingKeyUserInviteRegistrationEnabled] == "true",
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "天才程序员拼车站"),
		SiteLogo:                         settings[SettingKeySiteLogo],
		SiteSubtitle:                     s.getStringOrDefault(settings, SettingKeySiteSubtitle, "一群程序员共建的高质量 AI API 拼车站"),
		APIBaseURL:                       settings[SettingKeyAPIBaseURL],
		ContactInfo:                      settings[SettingKeyContactInfo],
		ContactQRCodeURL:                 settings[SettingKeyContactQRCodeURL],
...
type PublicSettingsInjectionPayload struct {
	...
	UserInviteRegistrationEnabled    bool                     `json:"user_invite_registration_enabled"`
...
func (s *SettingService) GetPublicSettingsForInjection(ctx context.Context) (any, error) {
	...
	return &PublicSettingsInjectionPayload{
		...
		UserInviteRegistrationEnabled:    settings.UserInviteRegistrationEnabled,
...
func (s *SettingService) portalMode() string {
	if s == nil || s.cfg == nil {
		return config.PortalModeStandard
	}
	return config.NormalizePortalMode(s.cfg.PortalMode)
}
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyGmailVerificationBypassEnabled] = strconv.FormatBool(settings.GmailVerificationBypassEnabled)
	...
	updates[SettingKeyUserInviteRegistrationEnabled] = strconv.FormatBool(settings.UserInviteRegistrationEnabled)
...
// IsGmailVerificationBypassEnabled 检查是否允许 Gmail/Googlemail 注册免邮箱验证码
func (s *SettingService) IsGmailVerificationBypassEnabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyGmailVerificationBypassEnabled)
	if err != nil {
		return false
	}
	return value == "true"
...
// IsRegistrationIPLimitEnabled 检查是否启用注册 IP 唯一限制
...
// IsUserInviteRegistrationEnabled 检查普通用户邀请码是否可用于通过注册门槛。
func (s *SettingService) IsUserInviteRegistrationEnabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyUserInviteRegistrationEnabled)
	if err != nil {
		return false // 默认关闭，避免用户邀请码绕过管理员邀请码策略
	}
	return value == "true"
}
...
// GetRawSetting returns the persisted raw setting value for a key.
func (s *SettingService) GetRawSetting(ctx context.Context, key string) (string, error) {
	if s == nil || s.settingRepo == nil {
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyGmailVerificationBypassEnabled:            "false",
		SettingKeyRegistrationEmailSuffixWhitelist:          "[]",
		SettingKeyRegistrationIPLimitEnabled:                "false",
		SettingKeyPromoCodeEnabled:                          "true", // 默认启用优惠码功能
		SettingKeyUserInviteRegistrationEnabled:             "false",
		...
		SettingKeySiteSubtitle:                              "一群程序员共建的高质量 AI API 拼车站",
		SettingKeySiteLogo:                                  "",
		SettingKeyPurchaseSubscriptionEnabled:               "false",
		SettingKeyPurchaseSubscriptionURL:                   "https://pay.ldxp.cn/shop/CN8U85FN",
