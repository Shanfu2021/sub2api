export default {
  ...
  auth: {
    ...
    promoCodeValidWithDiscount: '有效！注册后将获得 {factor}x 专属倍率',
    promoCodeValidWithBonusAndDiscount: '有效！注册后将获得 ${amount} 赠送余额，并享受 {factor}x 专属倍率',
  ...
  affiliate: {
    ...
    tips: {
      ...
      line3: '在线余额充值、余额卡/兑换码兑换、管理员手动正数余额加款都参与返利；订阅购买不参与，返利无时间限制、无金额上限。',
  ...
  redeem: {
    ...
    needToBuyTitle: '还没有兑换码？',
    needToBuyDesc: '可以先购买余额兑换码，购买成功后回到本页输入兑换码即可到账。',
    buyNow: '去购买余额卡',
    buyExternalHint: '将在新窗口打开购买页面',
    buyInternalHint: '跳转到站内购买页面',
    productSectionTitle: '购买通用余额卡',
    productSectionDesc: '购买成功后复制兑换码，回到上方输入即可到账。',
    productStoreNoticeTitle: '链动小铺店铺',
    productStoreNoticeDesc: '上方是我们推荐的常用余额卡，想看全部商品可以直接进入链动小铺店铺首页。',
    viewAllProducts: '查看全部商品',
    buyThisProduct: '查看并购买',
    productCategoryBalance: '余额卡',
    productCategorySubscription: '订阅卡',
    productBalanceTitle: '通用余额卡',
    productBalanceDesc: '适合按需充值。购买后获得兑换码，到账后直接增加账户余额。',
    productBalancePrice: '卡面按美元额度发放',
    productBalanceNote: '例如卡面 10 对应到账 100 美元额度，手续费不影响到账面额。',
    productBalanceBadge: '余额',
    productSubscriptionTitle: '订阅卡',
    productSubscriptionDesc: '适合长期使用。购买后使用兑换码开通对应的订阅套餐或时长。',
    productSubscriptionPrice: '按店铺展示的套餐面额购买',
    productSubscriptionNote: '不同商品可能对应不同订阅档位或有效期，请以下单页面说明为准。',
    productSubscriptionBadge: '订阅',
    currentBalance: '当前余额',
    concurrency: '并发数',
    requests: '请求',
    redeemCodeLabel: '兑换码',
    ...
    promoCodeHint: '如果你拿到了特殊折扣优惠码，可以在这里补录',
    applyingPromo: '应用中...',
    applyPromoButton: '应用优惠码',
    promoBonusApplied: '赠送余额已到账',
    promoDiscountApplied: '专属倍率已生效：{factor}x',
    promoApplySuccess: '优惠码应用成功！',
    redeeming: '兑换中...',
    redeemButton: '兑换',
    redeemSuccess: '兑换成功！',
    redeemFailed: '兑换失败',
  ...
  admin: {
    ...
    accounts: {
	  ...
	  autoPauseThresholdHint: '留空或填 0 表示使用全局默认阈值（在运维设置中配置）；填具体值则覆盖全局默认。达到阈值后会临时暂停到窗口重置时间，7d 到点后自动探测恢复。',
    ...
    promo: {
      ...
      discountLabel: '折扣文案',
      discountScope: '折扣范围',
      discountScopeAll: '余额和订阅都生效',
      discountScopeBalance: '仅余额生效',
      discountScopeSubscription: '仅订阅生效',
      maxUses: '最大使用次数',
      zeroUnlimited: '0 = 无限制',
      expiresAt: '过期时间',
      notes: '备注',
    ...
    ops: {
      ...
      settings: {
        ...
        openaiQuotaAutoPauseHint: '当 OpenAI OAuth 账号 5h / 7d 用量达到阈值时，系统会临时暂停到窗口重置时间；7d 到点后低并发探测一次，确认恢复后再放回调度。账号级阈值优先于此全局默认值。',
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          title: '邀请返利',
          description: '老用户邀请新用户注册；仅被邀请用户前 3 次余额加款会按比例返利，在线余额充值、余额卡兑换与管理员手动正数余额加款参与，订阅不参与。默认关闭。',
          enabled: '启用邀请返利',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 Sub2API',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">Sub2API 是一个强大的 AI 服务拼车管理平台，让您轻松管理和分发 AI 服务。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的服务套餐（VIP、免费试用等）</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务商账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
